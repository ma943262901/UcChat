...
public class ConversationListActivity extends BaseActivity implements OnItemClickListener, IConversationListener {
		...
		BaseApplication.getInstance().connectUCSManager(token, new ILoginListener() {
			...
			public void onLogin(final UcsReason arg0) {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						if (arg0.getReason() == 0) {
							ToastUtil.show(ConversationListActivity.this, "连接服务器成功");
						} else {
							ToastUtil.show(ConversationListActivity.this, "连接服务器失败");
						}
					}
				});
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if ((System.currentTimeMillis() - mExitTime) > TIME_DIFF) {
				ToastUtil.show(ConversationListActivity.this, "再按一次退出程序");
